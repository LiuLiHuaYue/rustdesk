  ...
  static Future<String> sync() async {
    try {
      ...
      final versionUri = Uri.parse(
        "$serverAddress/getConfigVer?token=$versionTokenB64&tsp=$versionTspB64&sign=$versionSignB64",
      );
      ...
      final configUri = Uri.parse(
        "$serverAddress/fetchServerConfig?token=$configTokenB64&tsp=$configTspB64&sign=$configSignB64",
      );
...
class _ActivationDialogState extends State<ActivationDialog> {
  ...
  Future<void> _onRegister() async {
    ...
    try {
      final result = await AppServices._sendAuthRequest(
        'registerAuth',
        widget.uid,
        activationKey,
      );
      ...
      if (success) {
        await AppServices.saveAuthKey(activationKey);
